  ...
  return (
    <div className="space-y-8">
      ...
      <div className={`border ${c.cardStatic} rounded-lg p-6 space-y-4`}>
        ...
        {tab === "canon" && (
          <>
            <p className="text-sm text-muted-foreground">
              {tr("母本可以来自已有 Novel Creation 书籍，也可以直接上传外部 TXT、Markdown 或 PDF 小说。", "Use an existing Novel Creation book or upload an external TXT, Markdown, or PDF novel as canon.")}
            </p>
        ...
        {status && (
          <div className={`text-sm px-3 py-2 rounded-lg ${status.startsWith("Error") ? "bg-destructive/10 text-destructive" : "bg-success-soft text-success"}`}>
            {status}
          </div>
        )}
      </div>
