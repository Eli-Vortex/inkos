...
const HOOK_TYPE_COLOR: Record<string, string> = {
  "主线伏笔": "bg-warning-soft text-warning dark:text-warning",
  "角色前置": "bg-success-soft text-success dark:text-success",
  "情感线伏笔": "bg-primary-soft text-primary dark:text-primary",
  "次要伏笔": "bg-info-soft text-info dark:text-info",
};
      ...
      {hooks.map((hook) => (
        <div key={hook.id} className="rounded-lg bg-secondary/30 px-3 py-2.5">
          <div className="flex items-center gap-1.5 mb-1.5 flex-wrap">
            ...
            {hook.promoted === true && (
              <span className="text-[12px] px-1.5 py-0.5 rounded-full bg-success-soft text-success dark:text-success">
                {tr("活跃", "Active")}
              </span>
            )}
            {hook.type && (
            ...
            {hook.core && (
              <span className="text-[12px] px-1.5 py-0.5 rounded-full bg-warning-soft text-warning dark:text-warning">
                {tr("核心", "Core")}
              </span>
            )}
            {hook.payoff && (
