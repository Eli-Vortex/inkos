...
function StageIcon({ status }: { status: PipelineStage["status"] }) {
  switch (status) {
    ...
    case "completed":
      return <CheckCircle2 size={14} className="text-success dark:text-success shrink-0" />;
  }
}
...
function ChapterRevisionPreview({ exec }: { exec: ToolExecution }) {
  ...
  return (
    <div
      ...
      className={`mx-3 mb-3 mt-1 rounded-xl border px-3 py-2.5 ${passed ? "border-success/30 bg-success-soft" : "border-warning/35 bg-warning-soft"}`}
    >
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className={`rounded-full px-2 py-0.5 text-[12px] font-semibold ${passed ? "bg-success-soft text-success" : "bg-warning-soft text-warning"}`}>
          {!details.applied
          ...
          {exec.logs && exec.logs.length > 0 && (
            <ul className="space-y-0.5">
              {exec.logs.map((log, i) => {
                ...
                return (
                  <li key={i} className={`text-xs font-mono break-words ${isError ? "text-destructive" : isWarn ? "text-warning dark:text-warning" : "text-muted-foreground"}`}>
                    {log}
                  </li>
                );
              })}
...
function UtilityExecStatusIcon({ status }: { status: ToolExecution["status"] }) {
  switch (status) {
    case "completed":
      return <CheckCircle2 size={10} className="text-success dark:text-success shrink-0" />;
    case "error":
  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen}>
      <CollapsibleTrigger className="flex items-center gap-2 px-2 py-1 rounded-lg hover:bg-muted/50 transition-colors cursor-pointer text-xs text-muted-foreground">
        ...
        {allDone && !hasError && <CheckCircle2 size={10} className="text-success dark:text-success" />}
