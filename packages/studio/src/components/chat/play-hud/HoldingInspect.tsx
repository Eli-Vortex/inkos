        ...
        <div className="min-w-0 space-y-3 px-3 py-3">
          ...
          {lifecycle ? (
            <div>
              ...
              {lifecycle.reason ? <p className="mt-1.5 break-words text-[13px] leading-5 text-success">▲ {lifecycle.reason}</p> : null}
            </div>
          ) : row.statusPill ? (
            <div>
              ...
              {row.isFresh ? (
                <p className="mt-1.5 text-[13px] leading-5 text-success">▲ {isZh ? "刚获得" : "Just acquired"}</p>
              ) : row.changeReason ? (
                <p className="mt-1.5 break-words text-[13px] leading-5 text-success">▲ {row.changeReason}</p>
              ) : null}
            </div>
          ) : null}
