...
describe("storyboard creation runner", () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), "novel-creation-storyboard-assets-"));
  ...
  it("applies storyboard-specific Skill guidance without reusing the long-writing Skill", async () => {
    ...
    expect(messages[0]?.content).toContain("novel-creation-storyboard");
    expect(messages[0]?.content).toContain("Translate narrative beats into visible shots.");
    expect(messages[0]?.content).not.toContain("novel-creation-long-writing");
  });
