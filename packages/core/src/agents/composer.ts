...
import { parseChapterFileNumber } from "../state/chapter-workspace.js";
...
async function applyContextBudgetIfNeeded(params: {
  ...
  if (protectedTokens > availableInputTokens) {
    ...
    throw new Error(
      ...
      "Novel Creation will not compress protected author intent, current focus, hard state, or active hook evidence.",
    );
  }
...
export class ComposerAgent extends BaseAgent {
    ...
    const response = await this.chat([
      {
        role: "system",
        content: [
          "You are Novel Creation's semantic story-memory selector.",
  ...
  async selectOutlineSections(request: OutlineSectionSelectionRequest): Promise<ReadonlyArray<string>> {
    ...
    const system = isEn
      ? [
          "You are Novel Creation's semantic outline-section selector.",
  ...
  async selectReferenceSections(request: ReferenceSectionSelectionRequest): Promise<ReadonlyArray<string>> {
    ...
    const system = isEn
      ? [
          "You are Novel Creation's semantic reference-section selector.",
  ...
  async compileCompressibleContext(request: CompressibleContextCompileRequest): Promise<string> {
    ...
    const system = isEn
      ? [
          "You are Novel Creation's semantic context compiler.",
  ...
  chapterNumber: number,
    ...
    try {
      ...
      const chapterFiles = files
        .map((file) => ({ file, num: parseChapterFileNumber(file) }))
        .filter((entry): entry is { file: string; num: number } => entry.num !== null && entry.num < chapterNumber)
