...
describe("tui layout", () => {
  it("renders a codex-like single-column workspace preview", () => {
    ...
    expect(frame).toContain("告诉 Novel Creation");
  });
  ...
  it("keeps the two-line status strip above the composer preview", () => {
    ...
    expect(frame.indexOf("task.completed: Completed write_next for harbor.")).toBeLessThan(frame.indexOf("Ask Novel Creation"));
    expect(frame.indexOf("Mode auto")).toBeLessThan(frame.indexOf("Ask Novel Creation"));
  });
